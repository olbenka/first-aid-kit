...
import ru.project.service.AppUserService;
...
public class AppUserServiceImpl implements AppUserService {
    ...
    public String registerUser(AppUser appUser) {
        if (appUser.getIsActive()) {
            ...
            return "Процесс регистрации уже был запущен. На почту отправлено письмо. \n" +
                    "Перейдите по ссылке в нем для подтверждения регистрации.";
        }
    ...
    public String setEmail(AppUser appUser, String email) {
        try {
            ...
            if (optional.isEmpty()) {
                ...
                return "На почту было отправлено письмо. Перейдите по ссылке для подтверждения регистрации.";
            } else {
                return "Этот email уже используется. Введите корректный email. \n Для отмены команды введите /cancel";
            }
        } catch (AddressException e) {
            return handleEmailValidationError(appUser);
        } catch (Exception e) {
            return handleEmailSaveError(appUser);
        }
    }

    private String handleEmailValidationError(AppUser appUser) {
        appUser.setUserState(UserState.WAIT_FOR_EMAIL_STATE);
        appUserDAO.save(appUser);
        return "Введите корректный email. Для отмены команды введите /cancel";
    }

    private String handleEmailSaveError(AppUser appUser) {
        appUser.setUserState(UserState.WAIT_FOR_EMAIL_STATE);
        appUser.setEmail(null);
        appUserDAO.save(appUser);
        return "Произошла ошибка при обработке email.";
    }
